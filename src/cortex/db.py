...
def _runtime_schema_name() -> str:
    configured = load_config().database.schema_
    return _validated_schema_name(os.environ.get("CORTEX_DB_SCHEMA_OVERRIDE", configured))
...
def init_schema(drop: bool = False) -> None:
    ...
    cfg = load_config()
    override = os.environ.get("CORTEX_DB_SCHEMA_OVERRIDE")
    schema = _validated_schema_name(override or cfg.database.schema_)
    if drop and (not override or not re.fullmatch(r"cortex_test_[a-z0-9_]+", override)):
        raise PermissionError("drop=True 仅允许显式的 cortex_test_* 测试 schema；默认 cortex 永不删除")
    eng = get_engine()
    with eng.begin() as conn:
        ...
        if schema != "cortex":
            sql = sql.replace("cortex.", f"{schema}.")
            sql = sql.replace("CREATE SCHEMA IF NOT EXISTS cortex", f"CREATE SCHEMA IF NOT EXISTS {schema}")
            sql = sql.replace("SET search_path = cortex", f"SET search_path = {schema}")
        conn.execute(text(sql))
        # 运行时维度二次校验:查 entities.embedding 实际 vector 维度
        row = conn.execute(text(
            "SELECT format_type(a.atttypid, a.atttypmod) FROM pg_attribute a "
            "JOIN pg_class c ON c.oid=a.attrelid JOIN pg_namespace n ON n.oid=c.relnamespace "
            "WHERE n.nspname=:schema AND c.relname='entities' AND a.attname='embedding'"
