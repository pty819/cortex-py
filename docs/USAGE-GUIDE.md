...
> 📖 **scope 选错的后果不是报错，而是静默的数据隔离**（结构知识和故障诊断互相看不见、跨文档实体无法合并）。详见 [SCOPE-GUIDE.md](./SCOPE-GUIDE.md) 的决策清单。
...
**Q: scope 怎么选？**
A: 一个设备一个 scope（`equip:XXX-v1`）。如果有多台同型设备且经验需要共享，用祖先 scope（`equip:XXX-series`）灌通用知识，各自 scope 灌机台特定案例，recall 时用 `view=holistic` 向上查。完整决策清单和「灌错 scope 怎么补救」见 [SCOPE-GUIDE.md](./SCOPE-GUIDE.md)。
